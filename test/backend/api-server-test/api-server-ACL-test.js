...
describe('API tests', function() {
    ...
    before('Seed DB', function(done) {
        seedDB().then( _ =>
            Promise.all([
                Person.login({email: 'test-user1@appirio.com',
                    password: 'password'})
                    .then((token) => {
                        accessToken1 = token.id;
                    }),
                Person.login({email: 'test-user2@appirio.com',
                    password: 'password'})
                    .then((token) => {
    ...
    describe('GET /api/people', function() {
        describe('authenticated user', function() {
            ...
            it(`should not get another user\'s identity by ID`,
                function(done) {
                    request
                        .get(`/api/people/test-user1`)
    ...
    describe('GET /api/outfits', function() {
        ...
        describe('authenticated user', function() {
            ...
            it(`should not get another user's own outfit's garments`,
                function(done) {
                    request
                        .get(`/api/outfits/test-outfit1/garments`)
                        .set('Authorization', accessToken2)
    ...
    describe('GET /api/garment', function() {
        describe('authenticated user', function() {
            ...
            it.skip(`should get user's own outfit by ID`,
                function(done) {
                request
